...
export default function PurchaseReturn() {
  ...
  const [cashRecovered, setCashRecovered] = useState('')
  ...
  function addLine(g) {
    setLines((prev) => [...prev, { lot_id: g.lot_id, goods_name: g.goods_name, rate: Number(g.lot_value) || 0, tax_pct: Number(g.tax_percent ?? 0), qty: 1, available: Number(g.current_qty) }])
    setGoodsQuery('')
  }
  ...
  async function handleSubmit() {
    ...
    setResult(null)
    if (!accountId) {
      setError('Pick the supplier the goods are going back to.')
      return
    }
    if (lines.length === 0) {
    ...
    setSubmitting(true)
    const { data, error } = await supabase.rpc('create_purchase_return_voucher', {
      ...
      p_cash_recovered: cashRecovered === '' ? 0 : Number(cashRecovered),
    })
    setSubmitting(false)
    if (error) {
  ...
  if (!canAdd('purchase_return')) {
    return (
      <section>
        <div className="page-head"><h1>Purchase Return</h1></div>
        <p className="empty-note">Your account doesn't have permission to create purchase returns.</p>
      </section>
    )
  }
    ...
    <section>
      <div className="page-head">
        <div>
          ...
          <p className="sub">Goods going back to a supplier — reverses the original purchase's stock and ledger effect</p>
        </div>
      </div>
      ...
      <div className="voucher-head-grid">
        ...
        <div className="field" style={{ marginTop: 0 }}>
          <label>Original Purchase Voucher No. (optional)</label>
          <input value={referenceNo} onChange={(e) => setReferenceNo(e.target.value)} placeholder="e.g. P-M000101" />
        </div>
      </div>
      ...
      <div className="voucher-grid">
                ...
                {computed.map((l, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="num">
                      ...
                      {l.available != null && <div style={{ fontSize: '.65rem', color: 'var(--muted)' }}>of {l.available} in stock</div>}
                    </td>
        ...
        <div className="card">
          ...
          <div className="summary-row total"><span>Total debit</span><span className="val tnum">{sym}{money(grandTotal)}</span></div>
          <div className="field">
            <label>Cash recovered now (rest reduces what you owe them)</label>
            <input type="number" value={cashRecovered} onChange={(e) => setCashRecovered(e.target.value)} placeholder="0" />
          </div>
